...
int iiDFA(unsigned char trama[MTS], int size, unsigned char Aesperado, unsigned char Cesperado)
{
	...
	unsigned char bcc2 = xor(trama, 4, size-3);
	
	//printf("BCC2: 0x%X\n", bcc2);
	//printf("BCC2 trama: 0x%X\n", trama[size - 2]);
...
//editar esta funcao e o llwrite() e llread();
int enviarTramaIParaRR_REJ(int fd, unsigned char frame[MTS], int size, unsigned char frameLido[MTS],
	unsigned char Aesperado, unsigned char Cesperado1, unsigned char Cesperado2)
{
   			...
   			if ( estadoReceber >= 0 ) // se leu alguma coisa
   			{
   				//printf("%d\n", estadoReceber);
					//printf("ESTADO >= 0\n");
  					...
  					estadoRR = maquinaEstados(frameLido, estadoReceber, Aesperado, Cesperado1);
  					estadoREJ = maquinaEstados(frameLido, estadoReceber, Aesperado, Cesperado2);
  					
  					if (estadoRR == 0 || estadoREJ == 0)
  					{
  						alarm(0); //desactivar alarme
  						return(estadoReceber);
  					}
  					else
  					{
  						KEEP = 0;
